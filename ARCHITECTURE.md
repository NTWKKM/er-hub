...
|| `tools/er-note/*.html` | Seven standalone per-template clinical note worksheets (`general-er-note`, `sepsis`, `trauma`, `mammalian-bite`, `chest-pain`, `abdominal-pain`, `eye-injury`). Each file is self-contained: no dependency on `shared/components.js` or `shared/form-validate.js`. Shared visual style and behavior are provided by local `er-note.css` and `er-note.js` (no base.css import). Features: tab-style cross-template navigation, auto-saving drafts to `localStorage` keyed by template, plain-text note generation and `navigator.clipboard`, clear draft, print via `window.print()` with `@media print` hiding nav/action bars, and built-in clinical calculators (e.g. NEWS2/SIRS/HEART/Alvarado/RIG dose). | `tools/er-note/er-note.css`, `tools/er-note/er-note.js` |
...
|| `service-worker.js` | PWA offline cache. Network-first for navigation, cache-first for assets. Caches all HTML/CSS/JS + 5 shared behavior modules (incl. `clinical-engine.js` per ADR-49) + ER NOTE templates (`tools/er-note/*.html`, `er-note.css`, `er-note.js`) + square 512×512 app icon + 5 source PDFs + Google Fonts (including `Inter Tight`). `CACHE_VERSION` is `er-hub-v22` (bumped for ER NOTE Part B template standardization in ADR-52; keep in sync with `index.html` nav-right version string). Enables full offline access during ED wifi outages. | None |
...
1. **Input Collection (`Src`):** User enters history, exam findings, investigations, and scoring variables per template. Drafts auto-save to `localStorage` under a per-template key (`ernote-draft-{templateId}`).
2. **Clinical Processing (`Transform`):** Embedded template-specific JavaScript computes risk scores and dosing aids (e.g. HEART score, Alvarado score, qSOFA/SIRS/NEWS2, RIG dose) and writes results back to read-only display fields.
3. **Output (`Dest`):** The same input values are rendered as a plain-text ER note for `navigator.clipboard` and as a print-friendly document via `window.print()` with `@media print` styles that hide navigation and action bars.
...
|| `ER NOTE Drafts` | Client-only per-template drafts persisted in `localStorage` (`ernote-draft-{templateId}`). Cleared on explicit "Clear Draft" action; never uploaded. | Drafts survive browser restarts until cleared or overwritten. |
