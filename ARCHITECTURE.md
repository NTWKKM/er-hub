...
`CACHE_VERSION` is `er-hub-v51` and `CACHE_DATE` is `19/08/2569` — dynamically extracted by `index.html` and `shared/components.js` at runtime via `fetch()` and regex parsing, backed by `localStorage` (`er-hub-cached-version`) for cold-load offline resilience.
