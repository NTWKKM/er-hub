...
- **Background:** `#f8f7f4` (Swiss warm paper screen background across worksheets and portal)
- **Container Background:** `#ffffff` (Clean card background / worksheets)
- **Primary Text / Ink:** `#111827` (Primary slate text on portal and form headers)
- **Secondary Text / Graphite:** `#4b5563` (Secondary metadata, status badges, and arrows)
- **Border / Rule:** `#e5e7eb` (Subtle hairline dividers and card borders)
- **Signal Orange:** `#d84315` (Reserved for time-critical visual status dots on portal: rt-PA, STEMI, Massive PE, Anaphylaxis, RSI, Resus Timer, MgSO4, Burns. Acts as a high-visibility clinical triage marker meeting WCAG AA 3:1 contrast requirements).
- **Portal Hover / Slate Blue:** `#3b5998` / `#f1f5f9` (Subtle background transition with colored left sentinel on portal rows).
- **Braun White:** `#F0EDE5` (Warm ivory used for navigation bar text — `nav-home`, `nav-title`, `nav-center`, `nav-right`).
...
- **Text Primary:** `--text-primary: #111827` (slate-900 body text on form pages)
...
- **Font Sizes:**
  ...
  - Print Headers: `16px`
  - Portal Scale: `11.5px` (badges/eyebrows), `13.5px` (category/metadata/numbers), `15px` (titles/headers), `22px` (brand display)

### Breakpoints & Layout

- **Container Max-Width:** `1100px` (order pages), `1120px` (portal Swiss grid)
- **Form Grid Gaps:** `20px` (STEMI/NSTEMI), `30px` (Stroke)
- **Portal List Layout:** Clean, minimalist 4-tier clinical hierarchy without search clutter or 3D lag:
  1. `⚡ Time-Critical Fast-Track & Resuscitation` (rt-PA, STEMI, PE, Anaphylaxis, RSI, Resus Timer, MgSO4, Burns) with blinking signal LEDs.
  2. `💊 Standing Orders & Inpatient Pathways` (NSTEMI, Heparin, Antivenom, Sedation).
  3. `🧮 Bedside Dosing & Clinical Calculators` (IV Drip Calculator, Clinical Score Hub, TB Dosing Calculator).
  4. `📋 Clinical Documentation & Notes` (ER NOTE Suite, NIHSS Score Sheet, Urgent Clinic Home Medication).
  Each section is an isolated card block with a header, structured semantic `<ol class="order-list">` with min-height 52px rows, 4px left-sentinel border on hover, and smooth 100ms transitions.
