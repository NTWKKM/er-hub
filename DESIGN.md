...
| **Portal 3D Flip Card** | Dual-version interactive card container (`.flip-card-container`) for protocols and tools available in both V1 and V2 (rt-PA `01`, NSTEMI `02`, NIHSS `T2`, Antivenom `06`). Front face defaults to V1 (`orders/rtpa.html`, `orders/nstemi.html`, `tools/nihss.html`, `orders/antivenom.html`); back face houses V2 (`orders/rtpa-v2.html`, etc.). Version switch button (`.version-flip-btn`) triggers a 520ms `rotateY(180deg)` flip animation (`cubic-bezier(0.2, 0.8, 0.2, 1)`) without navigating away, while clicking the card body follows the currently active version. Dynamic `tabindex` and `aria-label` synchronization ensure 100% keyboard and screen-reader accessibility. | **Default State:** V1 front face active (`.is-flipped` false, front `tabindex="0"`, back `tabindex="-1"`). **Flipped State:** `.is-flipped` true (`rotateY(180deg)`, front `tabindex="-1"`, back `tabindex="0"`). **Motion:** Suppressed via `transition: none !important` under `@media (prefers-reduced-motion: reduce)`. |
...
| **Top Navigation Bar** | Sticky full-width bar injected by `ED_COMPONENTS.injectNavBar()` (and statically on `index.html`). Auto-detects page title from `document.title` (strips after `—`) unless explicitly overridden. Shows MNRH logo (38px height) on all pages. "Home" link + title text. Normalised standing order nav titles to include the page title, clinical guideline, and release version format. On viewports <=900px, dynamically parses and truncates the title (e.g. `NSTEMI V2.1.1` or `rt-PA Calc V2.0`) to prevent wrapping. Short title aliases in `parseTitle()`: strips `Standing Order`, `Protocol`, `Post-Intubation`, residual `Order` word; aliases `IV Infusion Drip Calculator → Drip Calc` and `rt-PA Dose Calculator → rt-PA Calc`. Right side features `#nav-ver-display` containing `#nav-ver-text` (dynamically synced from `service-worker.js` `CACHE_VERSION` and `CACHE_DATE`, e.g. `v52 · 19/08/2569`, cached in `localStorage` as `er-hub-cached-version`) and `#online-status` (8px round LED dot: `#27ae60` green when online, `#c0392b` red when offline, listening to `online`/`offline` window events). | Blue gradient (`#1e3c72 → #2a5298`), `position: sticky; top: 0; z-index: 100`. Text colour: Braun White `#F0EDE5` (flat, warm ivory) for `.nav-home`, `.nav-title`, `.nav-center`, `.nav-right` — explicitly styled on both `index.html` and `.top-nav .nav-right` across all standing orders and clinical tools. `width: calc(100% + (var(--page-pad) * 2))`, negative margins to escape body padding. Padding is `0 24px` (desktop), reduced to `0 16px` via media query at <=768px to ensure perfect horizontal alignment of the hospital logo across the portal and all order pages. Hidden in print via `@media print`. |
...
| **Patient Sticker Box** | Standardized dashed bounding box mimicking paper patient label stickers. | Screen: `width: 170px; height: 46px;` Print: `width: 48mm; height: 16mm;` |
...
| **GCS Calculator (Trauma)** | Eye / Verbal / Motor radio groups with auto-total in the Disability section of trauma template. | **Eye (E):** 4 Spont / 3 To voice / 2 To pain / 1 None. **Verbal (V):** 5 Oriented / 4 Confused / 3 Words / 2 Sounds / 1 None / T Intubated. **Motor (M):** 6 Obey / 5 Localize / 4 Withdraw / 3 Flex / 2 Extend / 1 None. **Total:** E + V + M (V=T counts as 1). Auto-selects trauma-d range radio (13-14 / 9-12 / ≤8) based on total. Score-line with `data-copy` for clipboard integration. |
| **NIHSS Input Validation** | Per-item max score clamping on blur for all 33 input cells across 3 assessment columns. | `itemMax` lookup table defines max valid score per NIHSS item (1a-3, 1b-2, 5a-4, etc.). `inputmode="numeric"` on all cells. Blur handler clamps out-of-range values to item max, clears non-numeric input, allows 'X' for untestable items (5a/5b/6a/6b/10). Total row shows "(max 42)". |
...
- **Grid Header:** `padding: 3px; font-size: 8pt; line-height: 1.1`. First-child header (Progress Note) gets `padding-top/bottom: 5px` for visual balance. Enforces transparent background (`background: transparent !important; background-color: transparent !important;`) on print across all clinical standing orders to eliminate toner-heavy gray shading.
...
- **Sticker Box:** Print dimensions `48mm × 16mm` (compact, calibrated for strict 1-page A4 print fit). Screen size `170px × 46px`.
...
- **Color:** All print output forced to black-on-white with `-webkit-print-color-adjust: exact`. All table and order grid headers (`.grid-header`) enforce transparent background (`background: transparent !important; background-color: transparent !important;`) across all standing order forms (STEMI, NSTEMI, Heparin, PE, Antivenom, Sedation, Anaphylaxis, rt-PA) to eliminate toner-heavy gray shading, ensure crisp ink-saver prints, and align with hospital physical medical record chart standards.
...
- **Float status box (mammalian-bite):** A fixed top-right box shows overall bite risk (LOW/MEDIUM/HIGH) plus 3 rows: Rabies (No PEP/Vax/RIG+Vax), Tetanus (None/Booster/Td+TIG), WHO Category (I/II/III). `updateBiteFloatStatus()` reads the rabies and tetanus plan text to determine per-row urgency: RIG+Vaccine → high, Vaccine-only → medium, No PEP/None → low. Overall = highest of the two. Called from `updateRabies()` and `updateTetanus()` on every input change.
