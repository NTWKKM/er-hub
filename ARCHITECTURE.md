...
| `clinical-engine.js` | Shared clinical logic engine containing lookup tables and formulas for GRACE score and eGFR (CKD-EPI 2021) calculations. eGFR function includes input validation, case-insensitive sex normalization, and `Math.round()` return (parity with anticoag-engine.js per ADR-46). Killip lookup uses direct string key (no `String()` coercion). | None |
| `anticoag-engine.js` | Logic engine determining Heparin/LMWH doses and titration changes based on clinical indications. Contains `calcEGFR_CKDEPI2021` with identical behavior to `clinical-engine.js` (ADR-46 parity: input validation, case-insensitive sex, `Math.round()`). | None |
...
**Context:** NSTEMI was the only order page among the 5 `use-current-time` pages (pe, heparin, antivenom, nstemi, rtpa) that had the checkbox deleted in ADR-20 §3 ("Removed auto-time features") — but the ADR-20 rationale (removing auto-time for *troponin draw times*) did not apply to the *order date/time* field. The other 4 pages had the checkbox restored in a later audit (A8, W-08), but nstemi remained without it, forcing clinicians to manually write the date/time on the printed order. Additionally, the ER clinical workflow for NSTEMI prefers blank date/time by default (the order is prepared before patient arrival or vitals are confirmed), unlike the other 4 pages where auto-fill is the expected default.

**Decision:**
1. **Restored `use-current-time` checkbox** to `orders/nstemi.html`, rendered inline on the same row as the "1. ข้อมูลผู้ป่วย" heading via a flexbox wrapper (`display:flex; align-items:center; justify-content:space-between`). The heading sits on the left, the checkbox label on the right. Both share a continuous `border-bottom: 2px solid var(--cardiac-primary)` line (matching the existing `.theme-cardiac h3` style), with the wrapper providing `margin-bottom:15px` to space the patient fields below.
...
**Rationale:** Restores parity with the other 4 pages (W-08 compliance) while respecting the NSTEMI clinical workflow preference for blank-by-default date/time. The inline-on-heading layout saves vertical space — no separate row needed for a single checkbox. The unchecked default avoids printing a stale timestamp when the order is prepared in advance.

**Tests:** 199/199 pass — no test changes needed. The `use-current-time` checkbox is a UI-only toggle; no calculation or print logic test asserts its presence or default state.
