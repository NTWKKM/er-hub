...
| `service-worker.js` | PWA offline cache. Network-first for navigation, cache-first for assets. Caches all HTML/CSS/JS + 5 shared behavior modules (incl. `clinical-engine.js` per ADR-49) + ER NOTE templates (`tools/er-note/*.html`, `er-note.css`, `er-note.js`) + square 512×512 app icon + 5 source PDFs + Google Fonts (including `Inter Tight`). `CACHE_VERSION` is `er-hub-v21`. Enables full offline access during ED wifi outages. | None |
