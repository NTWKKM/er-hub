...
| **Wound Illustration** | Inline SVG anatomical skin cross-section illustrations on mammalian-bite category and tetanus wound type buttons. Shows skin layers (epidermis, dermis, subcutis, fascia, muscle) with wound depth and type visualized as disruption to those layers. | `.wound-illust` — `80×80px` container, SVG viewBox `100×80`. Skin layer colors: epidermis `#e8c4a0`, dermis `#e0b48a`, subcutis `#f5e6d3`, fascia `#e8d5c0`, muscle `#c44545`. Blood fill `#a02020` for transdermal wounds. Debris dots and necrotic tissue ellipses for tetanus-prone wounds. Muted anatomical labels. Integrates with `.btn-tile` — illustration appears above `.tile-label`. |
