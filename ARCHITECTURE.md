...
| `orders/*.html` | Specialized clinical worksheets (rt-PA, STEMI, NSTEMI, PE, Antivenom, Heparin, Sedation). All 7 files now use `ED_PRINT_BOOTSTRAP` for page lifecycle (show/clear/print-blank-direct) and `ED_BLANK_PRINT` for declarative blank-print resets. Page-specific JS reduced to: form submit handler (clinical logic) + ED_BLANK_PRINT.register(manifest) + event listeners for protocol-specific UI (e.g. fib card selection). | `shared/base.css`, `shared/print.css`, `shared/calc-engine.js`, `shared/components.js`, `shared/print-bootstrap.js`, `shared/blank-print-engine.js` |
