...
| `tools/er-note/index.html` | ER NOTE portal hub listing the 7 per-template ER NOTE worksheets in the clinical order: General ER Note, Sepsis, Trauma, Mammalian Bite, Chest Pain, Abdominal Pain, Eye Injury. Uses inline Braun palette, local navigation, draft badge, and standalone asset links. | `tools/er-note/er-note.css`, `tools/er-note/er-note.js`, `service-worker.js` |
| `tools/er-note/*.html` | Seven standalone per-template clinical note worksheets (`general-er-note`, `sepsis`, `trauma`, `mammalian-bite`, `chest-pain`, `abdominal-pain`, `eye-injury`). Each file is self-contained: no dependency on `shared/components.js` or `shared/form-validate.js`. Shared visual style and behavior are provided by local `er-note.css` and `er-note.js` (no base.css import). Features: tab-style cross-template navigation, multi-patient draft persistence via schema v2 registry (ADR-53), patient strip with HN field, floating sidebar draft manager (FAB + slide-in panel with filter/delete), Investigation/Treatment checkbox modules via `ErNote.renderInvestigation`/`ErNote.renderTreatment`, Thai-base history labels, plain-text note generation and `navigator.clipboard`, clear draft, print via `window.print()` with `@media print` hiding nav/action bars/patient-strip/sidebar, and built-in clinical calculators (e.g. NEWS2/SIRS/HEART/Alvarado/RIG dose). | `tools/er-note/er-note.css`, `tools/er-note/er-note.js` |
...
| `service-worker.js` | PWA offline cache. Network-first for navigation, cache-first for assets. Caches all HTML/CSS/JS + 5 shared behavior modules (incl. `clinical-engine.js` per ADR-49) + ER NOTE templates (`tools/er-note/*.html`, `er-note.css`, `er-note.js`) + square 512×512 app icon + 5 source PDFs + Google Fonts (including `Inter Tight`). `CACHE_VERSION` is `er-hub-v23` (bumped for ER NOTE redesign in ADR-53; keep in sync with `index.html` nav-right version string). Enables full offline access during ED wifi outages. | None |
| `manifest.json` | PWA manifest. App name, theme color (`#f4f2ec` Braun cream), background color (`#ebe7df` Braun paper), single square 512×512 icon (`docs/icon-512x512.png`, `purpose: any maskable`) for install/splash-screen compliance. Enables installable app + offline. | `docs/icon-512x512.png` |
...
For `tools/er-note/*.html`, the flow differs because these worksheets are narrative clinical notes rather than order sheets:
1. **Input Collection (`Src`):** User enters history, exam findings, investigations, and scoring variables per template. Drafts auto-save to `localStorage` under a registry-indexed key (`ernote-draft-{templateId}-{draftId}`), with a registry index at `ernote-registry` holding `{ id, template, hn, cc, updatedAt }` per draft (ADR-53 schema v2). The HN field in the patient strip identifies the patient; the URL `?draft={id}` selects which draft to load.
2. **Clinical Processing (`Transform`):** Embedded template-specific JavaScript computes risk scores and dosing aids (e.g. HEART score, Alvarado score, qSOFA/SIRS/NEWS2, RIG dose) and writes results back to read-only display fields. `ErNote.renderInvestigation()` and `ErNote.renderTreatment()` render structured checkbox modules from per-template presets.
3. **Output (`Dest`):** The same input values are rendered as a plain-text ER note for `navigator.clipboard` and as a print-friendly document via `window.print()` with `@media print` styles that hide navigation, action bars, patient strip, and sidebar.
...
| `ER NOTE Drafts` | Client-only multi-patient drafts persisted in `localStorage` via schema v2 registry (`ernote-registry` index + `ernote-draft-{template}-{id}` per-draft data). Each draft carries patient HN and CC for sidebar identification. Cleared on explicit "Clear Draft" action or via sidebar delete; never uploaded. | Drafts survive browser restarts until cleared or deleted. v1 single-draft-per-template keys auto-migrated to v2 registry on first load (ADR-53). |
...
- **W-19: Deep Audit Remediation (2026-07-06, ADR-49):** Ten fixes from full-clone static audit. (1) Drip calculator keystroke-clamp moved from `'input'` to `'blur'` — clamping on every keystroke destroyed manual number entry (typing weight `45` → clamped to `30` → `45` appended to `30` → `3045` clamped to `250`). `recalculate()` also stopped writing clamped value back to `doseInput.value` during typing. (2) NSTEMI `updateLiveEGFR()` now delegates to null-safe `calcEGFR()` instead of calling `calcEGFR_CKD_EPI_2021` directly without null guard — typing `0.9` (starts with `0`) previously crashed with `null.toFixed()` TypeError. (3) `handlePrintBlankDirect()` no longer calls a second `window.print()` — the page's button handler already prints. (4) `shared/clinical-engine.js` added to SW `ASSETS` precache (was missing — would fail offline for NSTEMI GRACE/eGFR engine). `CACHE_VERSION` → `er-hub-v20`. (5) Dead `calcAnticoag` + duplicate `calcEGFR_CKDEPI2021` removed from `anticoag-engine.js` — `clinical-engine.js` is now the sole eGFR source of truth, eliminating the two-implementation drift class. (6) Dead `ED_COMPONENTS.updateDeptWard()` removed. (7) Dead CSS (`.flag-group`, `.nav-home-icon`, `.stroke-table` in shared files) removed. `dead-css-guard.test.js` extended to scan `shared/base.css` + `shared/print.css` against the full repo. (8) eGFR false precision `.toFixed(2)` → `.toFixed(0)` in 3 NSTEMI sites (engine returns integer). (9) STEMI age-75 boundary (TNK `age≥75` vs clopidogrel `age≤75`) confirmed intentional per clinical review — documented + 6 boundary tests added. (10) Vestigial `pnpm-lock.yaml` deleted (zero dependencies). 193/193 tests pass.
- **W-20: ER NOTE Tool Standalone & Per-Template Split (2026-07-07, ADR-50):** The ER NOTE tool was added as a new Clinical Tool category on the homepage (T2). It does not reuse `shared/components.js`, `shared/form-validate.js`, or `shared/base.css` — each template (`general-er-note`, `sepsis`, `trauma`, `mammalian-bite`, `chest-pain`, `abdominal-pain`, `eye-injury`) lives in its own HTML file, with shared style/behavior supplied by local `tools/er-note/er-note.css` and `tools/er-note/er-note.js`. The portal hub (`tools/er-note/index.html`) lists templates in the specified clinical order: General ER Note → Sepsis → Trauma → Mammalian Bite → Chest Pain → Abdominal Pain → Eye Injury. Each page persists drafts to `localStorage` (`ernote-draft-{templateId}`), generates a plain-text note to the clipboard, clears drafts on user action, and prints via `window.print()` with `@media print` hiding nav/action bars. Built-in calculators include NEWS2, SIRS/qSOFA, HEART score, Alvarado score, GCS, and RIG dose. `CACHE_VERSION` → `er-hub-v21`, `index.html` nav-right synced to v21.
- **W-21: Deep Audit Remediation B-1–B-4 (2026-07-07, ADR-51):** Four fixes from the 2026-07-07 deep audit. (1) `orders/antivenom.html`: `TetanusText` → `tetanusText` — the lowercase `let tetanusText` was declared but never read while three branches assigned to undeclared capital-T `TetanusText`, creating an implicit global in sloppy mode. Unified to a single `tetanusText` variable. (2) `orders/stemi.html`: added JS-side `ED_VALIDATE.range('age', 18, 100, ...)` and reduced HTML `max` from `120` to `100` to match the validation bound; previously only `ED_VALIDATE.min('age', 18)` existed, leaving an upper-bound gap inconsistent with other pages. (3) `manifest.json` + `service-worker.js`: replaced the non-square 379×262 logo icon with a square 512×512 icon (`docs/icon-512x512.png`, letterboxed on Braun cream `#F4F2EC`) and added it to the SW precache list; the legacy logo PNG remains cached as a content asset but is no longer the manifest icon. (4) Documented the existing non-blocking validation design as intentional: `ED_VALIDATE.range()`/`min()` highlight invalid fields but do **not** hard-block calculation/print on out-of-range values — this preserves clinician override ability in emergency workflows and is now explicitly recorded in W-21/ADR-51 to prevent future contributors from "fixing" it into a hard block.
- **W-22: ER NOTE Part B — Template Standardization (2026-07-08, ADR-52):** Refactored the 6 non-sepsis ER NOTE templates (`general-er-note`, `trauma`, `mammalian-bite`, `chest-pain`, `abdominal-pain`, `eye-injury`) to match the `sepsis.html` structural pattern: top nav + 7-link tab bar + `.card` sections + `.field-row` / `.radio-group` / `.checkbox-group` layouts + `.score-line` read-only score bars with `data-copy` attributes synced by inline `updateScores()` scripts + bottom `.action-bar` (Copy Note / Clear / Print) + `v22` footer. This eliminates layout drift between templates and guarantees `er-note.js` can build and copy a plain-text note from a consistent DOM shape. `CACHE_VERSION` → `er-hub-v22`; `index.html` nav-right version string synced to v22.
...
**Context:** A new Clinical Tool request (`2025-07-06_ER-NOTE-Tool.md`) required adding an ER NOTE worksheet with 7 templates. The original draft plan kept all templates in a single `template.html` shell with inline tabs. Two constraints emerged during implementation: (1) each template must live in its own file so changes do not require editing a monolithic page, and (2) the tool must not depend on the existing shared UI/validation modules because it is a narrative note tool with different UX needs (draft persistence, plain-text copy, print-friendly layout, scoring calculators).

**Decision:**

1. **F1 — Per-template split (Q1>A):** Created 7 standalone HTML files under `tools/er-note/`: `general-er-note.html`, `sepsis.html`, `trauma.html`, `mammalian-bite.html`, `chest-pain.html`, `abdominal-pain.html`, `eye-injury.html`. A portal hub (`tools/er-note/index.html`) lists the templates in the specified clinical order: General ER Note → Sepsis → Trauma → Mammalian Bite → Chest Pain → Abdominal Pain → Eye Injury. Each page carries its own tab bar linking to the other templates; switching reloads the page.
2. **F2 — Standalone assets (Q2>B):** Shared style and common behavior are provided by local `tools/er-note/er-note.css` and `tools/er-note/er-note.js`, not by `shared/base.css`, `shared/components.js`, or `shared/form-validate.js`. This isolates the ER NOTE visual language and lifecycle from the standing-order codebase, while still caching the files in the PWA service worker.
3. **F3 — Local draft persistence (Q3>B):** Each template auto-saves form values to `localStorage` under key `ernote-draft-{templateId}` on every `input`/`change`. A "Clear Draft" button removes the key and resets the form. Drafts are never transmitted to any server.
4. **F4 — Plain-text output (Q4>A):** The "Copy Note" button collects all non-empty fields, section labels, and computed score outputs into a single plain-text block and writes it to the clipboard via `navigator.clipboard.writeText()` (with a `textarea` fallback for insecure contexts).
5. **F5 — Print lifecycle (Q5>B):** "Print" calls `window.print()` directly. `@media print` rules hide the tab bar, action bar, and footer, leaving only the form content. There is no blank-order pathway; the printed page is the filled or partially filled note itself.
6. **F6 — Built-in calculators (Q6>C):** Each template embeds the score/dose logic it needs directly in its own file or calls helpers in `er-note.js`. Supported calculators: NEWS2, SIRS/qSOFA, HEART score, Alvarado score, GCS total, and RIG dose (≈ 2 mL/kg wound cleansing, max 100 mL per episode per WHO Thailand rabies guidance). Calculators write to read-only display fields and are included in copied/printed output.

**Rationale:** Splitting templates into separate files reduces merge conflicts and makes it easier for future clinical contributors to edit a single template without risking the others. Keeping assets local prevents the new tool from coupling to the standing-order component/validation contract, which would force the ER NOTE to inherit print-bootstrap and float-bar behavior that do not fit a narrative note. Local drafts give clinicians a safe recovery mechanism if the browser tab is closed mid-note. Plain-text copy matches real ED workflow: notes are pasted into the hospital EMR or a chat handoff. Built-in calculators reduce cognitive load and reduce arithmetic errors during resuscitation.

**Tests:** SW `CACHE_VERSION` → `er-hub-v21`; `index.html` nav-right synced to v21. Existing structural guards (dead-css-guard, dom-id-integrity-guard, order-safety-guard) are unaffected because the new files are not included in those guards by design — they scan only `orders/*.html`, `tools/drip-calculator.html`, and `index.html`.

---

### ADR-52: ER NOTE Part B — Template Standardization to Sepsis Pattern (2026-07-08)

**Context:** After ADR-50 shipped the 7 per-template ER NOTE files, the 6 non-sepsis templates (`general-er-note`, `trauma`, `mammalian-bite`, `chest-pain`, `abdominal-pain`, `eye-injury`) diverged in DOM structure from `sepsis.html`. Some lacked the tab bar, some used inconsistent section wrappers, score displays were not consistently marked for clipboard copying, and footer versions were mismatched. This created three risks: (1) `er-note.js` had to build plain-text notes from inconsistent DOM shapes, (2) future template edits would repeat the same layout decisions, and (3) version strings in `service-worker.js`, `index.html`, and template footers could drift.

**Decision:**

1. **F1 — Standardized page skeleton (Q1>A):** Every ER NOTE template now uses the same page-level structure: `top-nav` with hospital logo + "ER NOTE" title + Home link, a 7-link `.tab-bar` linking all templates, a sequence of `.card` sections, an `.action-bar` with Copy Note / Clear / Print buttons, and a `.footer` carrying the synchronized version string.
2. **F2 — Standardized field layouts (Q2>A):** All interactive fields are wrapped in `.field-row`; choice fields use `.radio-group` or `.checkbox-group`; inline vital-sign clusters use `.inline-row`. This makes CSS rules and `er-note.js` extraction logic reusable across all 7 templates.
3. **F3 — Score-line data-copy contract (Q3>A):** Every computed score or risk display uses `.score-box.score-line` with a `data-copy` attribute (e.g. `data-copy="qSOFA: 0/3"`). An inline per-template `updateScores()` script updates both the visible text and the `data-copy` value on every relevant `change` event. The "Copy Note" logic reads `data-copy` to include scores in the clipboard output without reverse-engineering the calculation.
4. **F4 — Version sync hardening (Q4>A):** Bumped `CACHE_VERSION` to `er-hub-v22` in `service-worker.js`, updated the `index.html` nav-right version string to `v22`, and set every template footer to `v22`. This forces a single coherent cache generation for the standardized templates.
5. **F5 — Scope boundary (Q5>A):** The refactor is limited to `tools/er-note/*.html`. It does not change the shared standing-order modules, the drip calculator, or the homepage behavior beyond the version string.

**Rationale:** Standardizing on the sepsis template as the reference shape makes the ER NOTE suite maintainable: one CSS rule applies everywhere, one JS extraction path applies everywhere, and future templates can be created by copying the reference file. The `data-copy` contract decouples visual rendering from clipboard output, preventing silent omissions when a score display is reformatted. Synchronizing version strings across SW, portal, and footers prevents stale-cache scenarios where a new template loads old assets.

**Tests:** `tests/nstemi-audit-fixes.test.js` updated to expect `er-hub-v22`. All 193 tests pass. Manual cross-template checks verified tab-bar links, footer version, and `data-copy` presence on every score line.
...
**Context:** The ER NOTE tool (ADR-50/52) had four limitations: (1) storage schema supported only 1 draft per template — opening a second patient with the same template overwrote the first draft; (2) investigations were free-text with no structure, mixing lab orders with results; (3) treatment had no consistent section across templates (except sepsis); (4) all field labels were pure English, including history-taking sections which Thai-speaking clinicians found slower to scan. The plan was validated against actual code and reviewed via 6 open questions resolved by the clinical lead.

**Decision:**

1. **F1 — Schema v2 multi-patient drafts (P0):** `er-note.js` rewritten with a registry-based storage model: `localStorage["ernote-registry"]` holds an index of `{ id, template, hn, cc, updatedAt }` entries; per-draft form data stored under `ernote-draft-<template>-<id>`. Lazy-create: no draft is created until the first field is entered. Migration from v1 (`ernote-draft-<template>`) auto-wraps old single drafts into the new registry on first load, preserving existing user data. URL `?draft=<id>` selects which draft to load.

2. **F2 — Patient strip + HN field (P1):** All 7 templates + index.html now have a `.patient-strip` div at the top of each form containing an HN input field. HN is used for sidebar card identification and is included in `copyNote()` output.

3. **F3 — Sidebar draft manager (P2):** Floating action button (FAB) on the right side of every ER NOTE page toggles a slide-in panel listing all drafts across all templates. Cards show HN, CC (truncated 40 chars), relative time, and a 4px left border colored by template (7 template-specific CSS vars). Real-time filter by HN or CC. Delete with `confirm()` dialog. "+ New Draft" button creates a new draft for the current template and navigates to it. Panel hidden in `@media print`.

4. **F4 — Investigation checkbox module (P3):** `ErNote.renderInvestigation(container, template)` renders Labs and Imaging checkbox groups per template-specific presets, plus a free-text row for items not in the preset. Presets incorporate clinical lead feedback: sepsis uses "H/C x 2" (hemoculture ×2) with LFT/UA/ECG added; abdominal pain adds Amylase alongside Lipase and replaces CXR with AAS (Acute Abdominal Series); Ca, Mg, PO4 added as optional items in all templates. `extractRow()` reads checkbox-groups and free-text separately — no custom merge needed. No result-entry fields (per requirement: note shows what was ordered, not results).

5. **F5 — Treatment checkbox module (P4):** `ErNote.renderTreatment(container, template)` renders treatment checkboxes + free-text. Templates with clinical-protocol-specific fields (sepsis ABx/fluid/vasopressor, mammalian-bite Rabies PEP/Tetanus) keep existing fields; only supportive treatment checkboxes are added. The 2-way input pattern (checkbox + free-text) ensures clinicians can document items not covered by presets.

6. **F6 — Thai-base history labels (P5):** History-taking sections (Chief Complaint/HPI, PMH/Medications/Allergies, ROS, and equivalent sections in specialty templates) now use Thai labels with English medical terms in parentheses. Chip text follows the same pattern (e.g. "เริ่มเป็นทันที (Sudden onset)"). Medical abbreviations (DM, HT, CKD, CAD, COPD, CVA, DLP, Asthma) remain in English inside chips. Exam, score, investigation, treatment, and disposition sections remain in pure English medical terminology.

7. **F7 — Version bump (P6):** `CACHE_VERSION` → `er-hub-v23`; `index.html` nav-right synced to `v23`; all ER NOTE template footers → `v23`.

**Rationale:** Schema v2 is a breaking change but migration ensures zero data loss — the most critical fix because the old single-draft-per-template model made the tool unusable for concurrent ED multi-patient workflows. The investigation/treatment modules standardize note structure while preserving clinical precision in protocol-specific templates. Thai-base history labels reduce scanning time for Thai-speaking clinicians during history taking, while keeping objective/exam/plan sections in English medical terminology for EMR interoperability.

**Tests:** 193/193 pass. ER NOTE files are excluded from structural guards (dead-css, id-integrity, order-safety) by design — these guards scan only `orders/*.html`, `tools/drip-calculator.html`, and `index.html`. The sidebar, render modules, and schema migration are exercised via manual cross-template verification.
