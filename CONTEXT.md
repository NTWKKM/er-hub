...
- **Decision:**
  1. **Clear-Render Fix:** Added `calculateAndRender()` call in `clear-btn` handler, after `updateLiveEGFR()` and before `applyBlankTemplate()`. Re-runs render pipeline with empty inputs → resets `#screen-grace` → `--`, `#screen-risk-label` → `--`, clears breakdown rows. Root cause: `form.reset()` clears input values but does not fire `input`/`change` events.
  2. **Print Listener Dedup:** Removed `ED_COMPONENTS.setupCommonActions()` call from NSTEMI init. `setupCommonActions()` in `components.js` wires `print-btn → window.print()`, duplicating the inline listener already in `nstemi.html`. Inline listener is now sole owner. `create-order-btn` listener retained (never in `setupCommonActions`). Same bug class as ADR-29/ADR-31 — duplicate re-introduced when `setupCommonActions()` was added back during a later refactor.
  3. **GRACE Summary Auto-Center:** Replaced `display: grid; grid-template-columns: 1fr 3fr 2fr` (ADR-42) with `display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 24px` on `.grace-summary`. Child `<div>`s use `flex: 0 1 auto` — each component sizes to natural width, row centers them as a group. Removed stale `.grace-summary { grid-template-columns: 1fr }` from `@media (max-width: 900px)` block. Margin changed from `20px 0` to `20px auto`.
- **Rationale:** `form.reset()` is not a substitute for re-rendering — same pattern as ADR-41 Finding 2. The double-print is a recurring regression: `setupCommonActions()` was designed for pages that don't wire their own print listener, but NSTEMI wires its own explicitly. The flex auto-center layout is more flexible than the rigid 1:3:2 grid and wraps gracefully on narrow viewports without a dedicated media query.
- **Verification:** Playwright headless test confirmed: `screen-grace` → `--`, `screen-risk-label` → `--` after clear; `print-btn` click → `window.print()` called 1 time (was 2). All 183 tests pass.

### ADR-41: NSTEMI Deep Audit & Fix — Print, Clear, Validation, Layout Reflow (2026-07-06)

- **Context:** A line-by-line audit of `orders/nstemi.html` and cross-file review identified 14 issues: (1) `print-blank-btn` never called `window.print()` (only applied blank DOM template); (2) Clear button left anticoag + DAPT panel in stale state after `form.reset()`; (3) no `ED_VALIDATE.range()` calls despite loading `form-validate.js` — dead HTML5 constraints + unreachable `form.submit` listener; (4) eGFR rounding inconsistency (`Math.round` vs unrounded) caused Enoxaparin frequency to contradict CI text at boundary 29.5–29.99; (5) `updateLiveEGFR()` missing `age >= 18` guard; (6) drip-calculator soft-clamp didn't write back to `doseInput.value`; (7–8) dead script includes (`index.html` → `components.js`, `drip-calculator.html` → `form-validate.js`); (10) `dead-css-guard.test.js` excluded `index.html`; (12) `manifest.json` theme colors didn't match Braun palette; (13) `index.html` redirect had no allow-list; (14) `showUpdateToast()` could attach duplicate click listeners. Post-deploy: cold-load triggered print dialog because `print-blank-btn.click()` was used programmatically for blank-template application (inherited `window.print()` from A1 fix). Stat Medications print lines needed 2-line layout (drug name + indented dose details).
- **Decision:**
  1. **NSTEMI button fixes:** `print-blank-btn` now calls `window.print()` when clicked by user. `applyBlankTemplate()` extracted as separate function for programmatic use (cold-load, clear-btn) — applies blank DOM template without triggering print dialog. Clear button resets anticoag panel (CI badges, disabled/override classes, hint messages, frequency container) and DAPT panel (P2Y12 radios, ASA checkbox, hint box) before re-applying blank template.
  2. **Validation:** `ED_VALIDATE.range()` calls added for weight (30–200), age (18–120), creatinine (0.1–20) inside `calculateAndRender()`. Dead `form.submit` listener removed. `MIN_AGE = 18` constant hoisted alongside `FONDA_MIN_EGFR`/`ENOX_MIN_EGFR`.
  3. **eGFR consistency:** `egfrForCalc = Math.round(egfr)` removed. Enoxaparin frequency auto-select now uses unrounded `egfr` directly, matching CI logic.
  4. **Age guard:** `updateLiveEGFR()` now checks `age >= MIN_AGE`, matching `calcEGFR()`'s existing guard.
  5. **Cross-file cleanup:** Drip-calculator input handler writes clamped value back to `doseInput.value`. `index.html` no longer loads `shared/components.js`. `drip-calculator.html` no longer loads `shared/form-validate.js`. `index.html` redirect validates `order` slug against `ALLOWED_SLUGS` array. `showUpdateToast()` guarded with `dataset.wired` flag. `manifest.json` theme/background colors → Braun palette (`#f4f2ec`/`#ebe7df`). `dead-css-guard.test.js` extended to include `index.html`.
  6. **Layout reflow:** `.grace-summary` (Row 1: Score + Breakdown) closes before new `.dapt-anticoag-row` grid (Row 2: DAPT left, Anticoagulant right). New CSS: `display: grid; grid-template-columns: 1fr 1fr; gap: 16px` with mobile breakpoint `grid-template-columns: 1fr` at ≤900px. Hidden in `@media print`.
  7. **Print changes:** Stat Medications block uses `.order-list-plain` (no bullets) with 2-line layout (drug name on line 1, dose details indented on line 2 via `<br><span style="padding-left:2em">`). Prasugrel print `<li>` uses normal text color (not red). Ticagrelor continuation wording → `1 x 2 pc`. Blank-print manifest entries synced.
  8. **Tests:** New `tests/nstemi-audit-fixes.test.js` (29 tests) covering all audit fixes. `dead-css-guard.test.js` +1 test for `index.html`. Total suite: 183/183 pass. `CACHE_VERSION` → `er-hub-v17`.
- **Rationale:** The `print-blank-btn` bug meant clicking "Blank Order" only refreshed the DOM but didn't actually print — a direct workflow contradiction. However, the same button's `.click()` was used programmatically for cold-load blank-preview and clear-btn reset — adding `window.print()` naively caused a print dialog on every page load. The `applyBlankTemplate()` extraction separates programmatic DOM reset from user-triggered print, fixing both issues. The clear-button stale state was the same bug class as ADR-10/ADR-29 (reset path not re-running all render functions). The eGFR rounding boundary bug was a silent clinical discrepancy where the auto-selected Enoxaparin frequency could contradict the CI text shown next to it. The layout reflow separates GRACE scoring (Row 1) from drug selection (Row 2), giving DAPT and Anticoagulant panels equal visual weight side-by-side. The 2-line Stat Medications layout improves readability — drug name on line 1, dose/quantity on line 2 with indentation. Removing bullets from Stat Medications matches the clean-print intent. Prasugrel's Cath-lab restriction is a workflow note, not a contraindication — normal text color is correct.
- **Evidence Base:** Plan validated against actual codebase state (ADR-40 DAPT panel already shipped — plan's B1 description was stale). rtpa.html `print-blank-btn` already had `window.print()`.
...
### ADR-36: Drip-Calculator Control & Layout Customizations (2026-07-05)
...
### ADR-37: Audit Cleanup, Dead CSS Elimination & Regression Guard (2026-07-05)
...
- **Decision:**
  ...
  4. **#4 (High) — SW precache missing `clinical-engine.js`:** `orders/nstemi.html` loads `shared/clinical-engine.js` (GRACE + eGFR — the core clinical logic of the NSTEMI worksheet, flagged ACTIVE on homepage) but it was absent from the `ASSETS` precache list. A clinician opening nstemi.html for the first time during a genuine wifi outage would get a page whose GRACE/eGFR engine fails to load. Fixed: added `'./shared/clinical-engine.js'` to `ASSETS`, bumped `CACHE_VERSION` v19→v20, synced `index.html` nav-right.
  5. **#5 (Medium) — Dead `calcAnticoag` + duplicate `calcEGFR_CKDEPI2021`:** `anticoag-engine.js` carried two eGFR implementations (`CLINICAL_ENGINE.calcEGFR_CKD_EPI_2021` and `calcEGFR_CKDEPI2021`) that were byte-for-byte logically identical today but had no shared source of truth — a future coefficient tweak applied to one and not the other would silently reintroduce the "two engines drift apart" bug class (ADR history: Fondaparinux cutoff drift). `calcAnticoag` was also never called by any order page. Fixed: deleted `calcAnticoag` + `calcEGFR_CKDEPI2021` from `anticoag-engine.js` (made `clinical-engine.js` the sole eGFR source of truth). Removed 17 dead tests.
  6. **#6 (Medium) — Dead `ED_COMPONENTS.updateDeptWard()`:** Defined in `components.js` but never called by any order page. Removed.
  7. **#7 (Medium) — Dead CSS + guard test gap:** `.flag-group`, `.nav-home-icon` (`base.css`), `.stroke-table` (`print.css`) were unreferenced. The existing `dead-css-guard.test.js` only scanned inline `<style>` blocks in HTML files — it missed shared CSS files entirely. Fixed: removed 3 dead selectors. Extended `dead-css-guard.test.js` to also scan `shared/base.css` and `shared/print.css` against all `.html`/`.js` files in the repo (strips `@import url(...)`, handles compound class strings like `'nav-title nav-title-full'`).
  8. **#8 (Low) — eGFR false precision:** 3 sites in `nstemi.html` displayed eGFR as `.toFixed(2)` (e.g. `"86.00 mL/min"`) while `calcEGFR_CKD_EPI_2021` returns `Math.round(egfr)` — an integer. Fixed: all 3 sites now use `.toFixed(0)` → `"86"` (bundled with #2 fix).
  ...
  10. **#10 (Low) — Vestigial `pnpm-lock.yaml`:** 9-line lockfile with zero dependencies (`importers: { .: {} }`). Deleted.
