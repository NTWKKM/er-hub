...
- **Collapsible Module Architecture & High-Contrast Theme Headers:** 5 core clinical sections with theme-colored headers (Amber/Flame for Fluid, Clinical Blue for Airway, Royal Purple for Toxicology, Crimson Red for Escharotomy/ABA, Forest Green for Painter), with Section 1 (Hero Fluid Resuscitation) open by default for rapid emergency charting, plus global "➕ ขยายทั้งหมด" / "➖ ยุบทั้งหมด" controls.
- **Interactive 6-Column Lund & Browder Body Mapper (Bottom Section):** Interactive SVG human body model (Anterior & Posterior) supporting 6 age brackets (`0`: <1y, `1`: 1–4y, `5`: 5–9y, `10`: 10–14y, `15`: 15–17y, `adult`: ≥18y) with dynamic pediatric visual scaling (head enlargement & limb shortening for infant/child models), dynamic region percentage overlays, click & drag-to-paint interaction, and real-time depth color-coding (1st, 2nd, 3rd/4th degree). Quick-link from TBSA banner automatically expands and scrolls to this module.
- **Direct % TBSA Entry & Sticky Summary Banner:** Prominent full-width summary banner with severity color shifts (amber/orange/red), persistent direct numerical % TBSA input & +1% Palm controls, quick-scroll link to the BSA painter, and automated ABA referral syncing.
...
- **Fluid Resuscitation Section (24-Hour Schedule):** Full-width, prominent dual calculation of ATLS 11th Consensus / Modified Brooke ($2\text{--}3\text{ mL/kg/\%}$) and Classic Parkland ($4\text{ mL/kg/\%}$), with dynamic post-8h catch-up scheduling, prehospital fluid deduction, and Holliday-Segar (4-2-1) pediatric maintenance ($D_5LR$ or $D_5 0.45\%\text{NaCl}$) strictly indicated for children $\le 30\text{ kg}$ with automated EMR note integration.
- **Urine Output & Shock Titration Assistant:** Real-time feedback for adult ($0.5\text{ mL/kg/hr}$), pediatric ($1.0\text{ mL/kg/hr}$), and electrical/pigmenturia ($100\text{ mL/hr}$ adult, $1.0\text{--}2.0\text{ mL/kg/hr}$ child) targets with $\pm 10\%\text{--}30\%$ auto-adjustment and interactive shock fluid bolus selector ($10\text{, } 15\text{, } 20\text{ mL/kg}$ warmed LR in 15–30 min).
...
Divided into two parts: Active releases (rt-PA, NSTEMI, and tools T1 to T8 marked with ACTIVE status, displayed first directly without a header) and a "Prototype" section (containing the remaining standing orders and tools T9 to T11 marked with PROTOTYPE status, which is collapsed by default and toggleable via a tactile 3D perspective accordion header).
