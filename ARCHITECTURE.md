...
| `lib/doc-utils.ts` | Shared utility for resolving document URLs from `NEXT_PUBLIC_BASE_PATH`. `resolveDocUrl(path)` centralizes the `process.env.NEXT_PUBLIC_BASE_PATH ?? ''` fallback. Used by SedationOrder, PeOrder, HeparinOrder, StemiOrder. | None |
...
| `app/orders/stemi/page.tsx` | STEMI Standing Order page. HTML print path + PDF blank. `handlePrintOrder` uses `window.print()` on rendered order markup; `handlePrintBlank` opens source PDF in new tab. Stale results cleared on validation early-return. TNK-specific fields (`elderly`, `bracketIdx`) only stored for TNK orders; SK orders use `false`/`-1`. | `DashboardLayout`, `StemiOrder` |
...
| `app/orders/sedation/page.tsx` | Post-Intubation Sedation page. HTML print path. Fentanyl + Midazolam dual-drug calculator. `handlePrintOrder` uses `window.print()` on rendered order markup. | `DashboardLayout`, `SedationOrder` |
...
| **Print Pathways** | Three pathways: (1) PDF blank (stemi, heparin, antivenom) — opens source PDF from `public/docs/` in new tab via `window.open()`. (2) HTML generated order print (rtpa, nstemi, pe, sedation, stemi) — `window.print()` on rendered order markup. (3) HTML blank print (rtpa, nstemi) — `window.print()` on rendered blank template. |
