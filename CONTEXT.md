...
- **Decision:** Replace `injectTopNav()` with `injectNavBar()`. The new method injects a `<nav class="top-nav">` sticky bar at `document.body` firstChild (full-width, above container). Auto-detects the page title from `document.title` (strips after `—`), eliminating per-page string args. Blue gradient (`#1e3c72 → #2a5298`) matches the portal header brand. `body` changed from `display: flex; justify-content: center` to `display: flex; flex-direction: column; align-items: center` to stack nav above container. `drip-calculator.html` cleaned up: removed hardcoded `.back-link` HTML + CSS, added `components.js` script tag + `injectNavBar()` call — now consistent with all other sub-pages. No `print.css` changes needed — existing `@media print` rules already hide `nav`, `.top-nav`, and `a[href*="index.html"]`.
