...
| `burn-engine.js` | Burn assessment & resuscitation engine. Exports `LUND_BROWDER_TABLE`, `ABA_REFERRAL_CRITERIA`, `AIRWAY_SAFETY_PEARLS`, `getLundBrowderAgeColumn`, `calculateTBSA`, `calculatePediatricMaintenance`, `calculateFluidRequirements`, `getTargetUrineOutput`, `getUrineOutputTitration`, `getCyanideAntidoteDosing`, `getCOAssessment`, `evaluateInhalationRisk`, `getRegionPercentages`. | None |
