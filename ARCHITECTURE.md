...
| `service-worker.js` | PWA offline cache. Network-first for navigation, cache-first for assets. Caches all HTML/CSS/JS + 3 shared behavior modules + logo PNG + 5 source PDFs + Google Fonts (including `Inter Tight`). `CACHE_VERSION` bumped to `er-hub-v13`. Enables full offline access during ED wifi outages. | None |
...
- **W-15: Homepage Braun × Mid-Century Modern Redesign & SW v13 (2026-07-04):** Revamped the portal dashboard with a Braun-restrained aesthetic. Replaced the 3-column card grid with semantic vertical lists using tabular-nums ordering, hairline separators, and muted text category badges. Staggered fadeInUp animations, lifts, shadows, and gradients are removed. The Signal Orange dot is reserved strictly for time-critical items (rt-PA, STEMI, Massive PE) to serve as a high-visibility clinical triage visual marker. The top-nav displays the original blue gradient background with white text, an increased 38px logo size, and normalized standing order navigation titles. Bumped service worker CACHE_VERSION to er-hub-v13 and registered Inter Tight.
