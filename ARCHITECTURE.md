...
- **W-06: A4 Print Fit:** All order pages use `@page { size: A4; margin: 8mm 10mm 10mm 10mm }` with `body { display: block !important }` to override the screen flex layout. The 5-column order grid drops `min-width: 900px` in print and uses `font-size: 8pt` to fit within 190mm printable width. Stroke-specific pages (rt-PA) use `width: 100%` with `page-break-before: always` for multi-page documents.
