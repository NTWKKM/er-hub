...
- **Decision:** Replace `injectTopNav()` with `injectNavBar(homeHref)`. The new method injects a `<nav class="top-nav">` sticky bar at `document.body` firstChild (full-width, above container). Auto-detects the page title from `document.title` (strips after `—`), eliminating per-page string args. Home link label is "Home" (English) with 🏠 icon. Accepts optional `homeHref` parameter for path flexibility (`'index.html'` on portal, defaults to `'../index.html'` on sub-pages). Blue gradient (`#1e3c72 → #2a5298`) matches the portal header brand. `body` changed from `display: flex; justify-content: center` to `display: flex; flex-direction: column; align-items: center` to stack nav above container. Nav added to `index.html` (portal) for visual unity across all 9 pages. `drip-calculator.html` cleaned up: removed hardcoded `.back-link` HTML + CSS, added `components.js` script tag + `injectNavBar()` call. No `print.css` changes needed — existing `@media print` rules already hide `nav`, `.top-nav`, and `a[href*="index.html"]`.
