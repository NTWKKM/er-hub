...
| `print-bootstrap.js` | Shared print/page lifecycle: `handlePrintBlankDirect()` (URL param detection), `showResults()` (unhide + float bar + scroll), `clearResults(formId, extraFn)` (reset + hide + focus), `getDateTimeHTML(useTime, date)`, `getBlankDateTimeHTML()`. Eliminates the identical DOMContentLoaded-end pattern duplicated across 7 order pages. | `components.js` |
| `blank-print-engine.js` | Declarative blank-print reset engine. Each order page registers a manifest of reset rules (`{ id, value }` for textContent, `{ id, html }` for innerHTML, `{ id, className }` for class override, `{ id, style }` for style props, `{ selector, checked }` for checkboxes). `apply()` executes all rules. Fixes the ADR-10 bug class at the root — adding a new protocol page is now a manifest array, not a hand-written reset block. | None |
...
| `orders/*.html` | Specialized clinical worksheets (rt-PA, STEMI, NSTEMI, PE, Antivenom, Heparin, Sedation). All 7 files now use `ED_PRINT_BOOTSTRAP` for page lifecycle (show/clear/print-blank-direct), `ED_BLANK_PRINT` for declarative blank-print resets, and `ED_VALIDATE` for non-blocking validation. Zero `alert()` calls remain. Page-specific JS reduced to: form submit handler (clinical logic) + ED_BLANK_PRINT.register(manifest) + event listeners for protocol-specific UI. | `shared/base.css`, `shared/print.css`, `shared/calc-engine.js`, `shared/components.js`, `shared/print-bootstrap.js`, `shared/blank-print-engine.js`, `shared/form-validate.js` |
