...
- **Grid Cell:** `padding: 3px; font-size: 8pt` (`9pt` for NSTEMI); `line-height: 1.3` (`1.45` for NSTEMI). Order list items: `margin-bottom: 3px` (`4px` for NSTEMI); `line-height: 1.3` (`1.45` for NSTEMI). Demographics and GRACE variable lines are wrapped in `white-space: nowrap` spans with shortened blank print dotted line lengths to guarantee labels and values never wrap to separate lines.
