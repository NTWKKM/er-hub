...
- **Responsive Breakpoints:**
  ...
  - `≤600px` (NSTEMI mobile): `.patient-fields` switches from `flex` to `display: grid; grid-template-columns: 1fr 1fr`. Patient field inputs: `max-width: none; width: 100%`. eGFR field (`egfr-field` class) and ASA Allergy field span `grid-column: span 2` (full-width rows). Troponin header checkbox resets `margin-left: auto → 0` to prevent overflow; H0/H1/H3 inputs stack vertically.
